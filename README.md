...
- Aday seçiminde ve execute öncesinde yalnız test sembolünde açık gerçek LONG/SHORT Futures pozisyonu olmadığı doğrulanır; diğer sembollerdeki pozisyonlar engel değildir ve onlara dokunulmaz. Giriş market/taker emridir; one-way modunda çıkış aynı `positionId` üzerinde `reduceOnly`, HEDGE modunda ise aynı `positionId` ve deterministic close-side ile yalnız gerçekleşen miktar kadar gönderilir. Test sembolünde exit sonrası kalan veya karşıt pozisyon varsa test `OPEN_ALARM` olur.
