...
- Execute öncesi MEXC hesabında açık Futures pozisyonu olmadığı ve position mode'un desteklendiği doğrulanır. Giriş market/taker emridir; one-way modunda çıkış aynı `positionId` üzerinde `reduceOnly`, HEDGE modunda ise aynı `positionId` ve deterministic close-side ile yalnız gerçekleşen miktar kadar gönderilir. Exit sonrası kalan veya karşıt pozisyon varsa test `OPEN_ALARM` olur.
